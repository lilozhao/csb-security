...
| 阶段 | 内容 | 优先级 | 预计测试用例 |
|------|------|--------|-------------|
| **M1 (P0)** | identity/（AID + AAT + 密钥轮换 + 信任等级收编） | 立即可用 | ~25 |
| **M2 (P1)** | handshake/ 五步握手 + authz/ 权限交集 + UAC | 核心功能 | ~20 |
| **M3 (P2)** | transport/ 会话密钥协商（ECDH）+ token 绑定 | 增强安全 | ~15 |
| **M4 (P2)** | audit/ 哈希链审计 + defense/ 重放防护 + 限流 | 合规需求 | ~20 |
| **M5 (P3)** | defense/ 异常检测 + 完整集成 csb-a2a-aip | 完整三方授权 | ~10 |
