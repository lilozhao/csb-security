...
// Layer 2 扩展: 信任升级（P0 骨架 · 2026-09-11）
//   账本是事实，等级是派生：EvidenceLedger(追加+哈希链+签名) → TrustStore(重放派生)
const evidenceLedger = require('./trust/evidence-ledger');
const trustStore = require('./trust/trust-store');
const collector = require('./trust/collector');

// P0-3 D1: 验证签名（约束-验证分离）
const verification = require('./verify/verify-signature');
const { VerifLog } = require('./verify/verif-log');
const { ValidatorReputation } = require('./verify/validator-reputation');
...
module.exports = {
  ...
  // Layer 2 扩展: 信任升级（P0 骨架）
  EvidenceLedger: evidenceLedger.EvidenceLedger,
  EVIDENCE_KIND: evidenceLedger.KIND,
  EVIDENCE_ACTIONS: evidenceLedger.ACTIONS,
  TrustStore: trustStore.TrustStore,
  TRUST_STORE_DEFAULTS: trustStore.DEFAULTS,
  EvidenceCollector: collector.EvidenceCollector,
  ...
  // P0-3 D1: 验证签名（约束-验证分离 · author ≠ validator）
  verification,
  canonicalize: verification.canonicalize,
  signVerification: verification.signVerification,
  verifyVerificationRecord: verification.verifyVerificationRecord,
  sigHash: verification.sigHash,

  // P0-3 D1: 验证记录上链存证（verif-log · 不可篡改可追溯）
  VerifLog,

  // P0-3 D4: 验证者信誉衰减（双证失准追责 + 冻结 + 复权）
  ValidatorReputation,
